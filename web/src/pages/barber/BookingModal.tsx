...
const STATUS_ACTIONS: Record<string, { status: string; label: string; color: string }[]> = {
  PENDING: [
    { status: 'CONFIRMED', label: 'Confirmar', color: 'bg-accent-500 text-white hover:bg-accent-600' },
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-4 sm:items-center" onClick={onClose}>
      ...
      >
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              {booking.customer.phone && (
                <a href={`tel:${booking.customer.phone}`} className="mt-0.5 flex items-center gap-1 text-sm text-zinc-400 hover:text-accent-500">
        ...
        <div className="max-h-[70vh] space-y-3 overflow-y-auto px-5 py-4">
          ...
          <div className="flex items-center justify-between border-t border-zinc-100 pt-1 dark:border-zinc-800">
            ...
            {booking.totalPrice === 0 && booking.customer.plan ? (
              ...
              <span className="font-bold text-accent-600">{formatCurrency(booking.totalPrice)}</span>
            )}
          </div>
        ...
        <div className="px-5 pb-5">
          <button
            onClick={onClose}
            className="w-full rounded-xl py-2 text-sm font-medium text-zinc-500 transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-800"
          >
            Fechar
          </button>
        </div>
